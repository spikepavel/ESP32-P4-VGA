...
/*
  VGA.h — header для VGA.cpp из примера.
  Требует, чтобы ScreenID был определён в проекте:
    int ScreenID = 0;
*/
...
public:
    ...
    Font(int charWidth, int charHeight, const unsigned char *pixels, int firstChar = 32, int charCount = 96)
        : firstChar(firstChar), charCount(charCount), pixels(pixels), charWidth(charWidth), charHeight(charHeight) {}
    bool valid(char ch) const { return ch >= firstChar && ch < firstChar + charCount; }
};
...
public:
    ...
    // блокирует до конца вертикального обновления (использование: рисовать -> vsyncWait())
    void vsyncWait();

    // получение буферов
    uint16_t* getDrawBuffer(); // основной (для рисования)
    uint16_t* getBackBuffer(); // запасной

    // базовые настройки текста/рисования
    int cursorX;
    int cursorY;
    int cursorBaseX;
    uint16_t frontColor;
    ...
    // примитивы рисования (реализация частично в cpp, можно расширить)
    ...
    // дополнительные графические функции (заготовки)
...
protected:
    // IRAM-safe static callbacks (registered with esp_lcd)
    ...
    // swap выполняется в задаче (не из ISR)
    void swapBuffers();

    // task to handle async copies and buffer swaps
    static void vga_task(void* arg);

    // синхронизация/ресурсы
    SemaphoreHandle_t _sem_vsync_end = NULL;   // сигнал завершения отображения кадра
    SemaphoreHandle_t _sem_gui_ready = NULL;   // сигнал что GUI готов к переключению
    SemaphoreHandle_t _sem_bounce_notify = NULL; // ISR -> task notify
    QueueHandle_t     _bounce_q = NULL;        // очередь BounceRecord для task
    BounceRecord      _bounce_rec;             // временная запись для задачи (используется в cpp task)
    ...
    int  _pixClock = 0;
    uint16_t *_frBuf[2] = { nullptr, nullptr };
    int _fbIndex = 0;
    int _Width = 0;
    int _Height = 0;
