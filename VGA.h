...
protected:
    ...
    // Семафор для ожидания завершения async memcpy
    SemaphoreHandle_t _sem_async_done = nullptr;
    ...
    int _lastBBuf = 0;
    // async memcpy handle for ESP32-P4
    async_memcpy_handle_t async_mem_handle = NULL;
    ...
    // --- Для асинхронной обработки копирования (bounce -> bounce_buf) ---
public:
    typedef struct {
        void* dst;
        void* src;
        size_t len;
        bool last; // признак последнего блока — вызвать swapBuffers после копии
    } CopyJob_t;

protected:
    QueueHandle_t _copyQueue = nullptr;
    TaskHandle_t _copyTaskHandle = nullptr;
    static const int COPY_QUEUE_LEN = 256; // можно уменьшить/увеличить при необходимости

    // фоновые обработчики
    static void copyTask(void* arg); // task для выполнения копий
    static bool IRAM_ATTR async_done_cb(async_memcpy_handle_t h, async_memcpy_event_t *event, void *cb_args);
};
